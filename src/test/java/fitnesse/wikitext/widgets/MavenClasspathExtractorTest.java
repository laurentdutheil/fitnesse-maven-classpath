...
public class MavenClasspathExtractorTest {
	...
	public void extractedClasspathIncludesTestScopeDependenciesCompile() {
		List<String> classpathEntries = mavenClasspathExtractor.extractClasspathEntries(pomFile, "compile", null);
		StringBuffer sb = new StringBuffer();
		for (String cpEntry : classpathEntries) {
			sb.append(cpEntry);
		}
	...
	public void extractedClasspathIncludesTestScopeDependenciesCompileProfile() {
		...
		List<String> classpathEntries = mavenClasspathExtractor.extractClasspathEntries(pomFile, "compile", profiles);
		StringBuffer sb = new StringBuffer();
		for (String cpEntry : classpathEntries) {
			sb.append(cpEntry);
		}
